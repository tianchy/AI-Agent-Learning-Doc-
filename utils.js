...
function parseMarkdown(text) {
    ...
    if (text === null) {
        return { metadata: {}, content: '<p class="text-red-500">Content not available (null input to parser).</p>' };
    }
    try {
        ...
        renderer.paragraph = function(text) {
            // This regex aims to correctly identify Chinese and English parts for styling.
            // It's a simplified approach. For robust CJK language segmentation, more advanced libraries might be needed.
            // Basic idea: Apply .font-chinese to segments that look like Chinese, .font-english to others.
            // This is complex to do perfectly with regex. For now, CSS will handle base fonts.
            // Specific emphasis for Chinese (bold/larger) vs English (smaller) is better handled with CSS rules targeting language.
            // The current CSS setup relies on Noto Sans SC for Chinese characters appearing larger/bolder by default at same font-size.
            // And .font-english for specific de-emphasis.
        ...
        renderer.heading = function (text, level, raw, slugger) {
            ...
            const id = `${slug}`; 
            // CSS classes will define sizes for h2, h3, h4, h5 from markdown in styles.css
            // All markdown headings will get .font-chinese for primary font
        ...
        renderer.table = function(header, body) {
         ...
         renderer.blockquote = function(quote) {
              return `<blockquote>${quote}</blockquote>\n`; // Will be mostly replaced by KeyConceptCard
         }
